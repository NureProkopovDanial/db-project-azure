                ...
                jwt.verify(token, config.secret, { issuer: config.issuer }, function (err, decodedToken) {
                    ...
                    if (decodedToken.CName) {
                        ...
                        if (DBConnectionsList[dbName]) {
                            ...
                            DBConnectionsList[dbName] = mongoose.createConnection(config.cosmosDB.connectionString, { dbName: dbName, // Указываем имя БД динамически useNewUrlParser: true, useUnifiedTopology: true });
